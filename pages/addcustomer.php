                                    ...
                                    <div class="col-lg-6">
                                        <form role="form" action = "" method = "POST">
											...
											<div class="form-group">
                                                ...
                                                <input type = "email" class="form-control" placeholder="Enter address" name = "email"required>
                                            </div>
											<div class="form-group">
										...
										<?php 
											if (isset($_POST['submit'])){
												...
												$sql = "SELECT * FROM customer_info WHERE customer_name = '".$_name."' AND customer_emal = '".$_email."'";
												$result = mysqli_query($link,$sql);
												$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
												$count = mysqli_num_rows($result);
												
												if($count == 1) {
													echo "<script>alert ('Customer information already exists, not saved')</script>";
													echo "<script>window.close(); </script>";
													exit();
													
												}else{
													$sql = "INSERT INTO customer_info (customer_name,customer_mob, customer_emal, customer_type) VALUES ('".$_name."','".$_mobile."', '".$_email."', '".$_type."')";
			
													if (mysqli_query($link, $sql)) {
														echo "<script>alert ('Customer information successfully saved')</script>";
														echo "<script>window.close(); </script>";
