                                    ...
                                    <div class="col-lg-6">
                                        <form role="form" action = "" method = "POST">
											...
											if (mysqli_num_rows($result) > 0) {
												...
												while($row = mysqli_fetch_assoc($result)) {
													echo "<option>".$row['tank_name']."</option>";
													$product = $row['product_name'];
												}
												echo "</select>
												</div>";
											}else{
												echo "<script>alert ('Add tank')</script>";
												echo "<script>self.location = 'addtank.php'</script>";
												exit();
											}
										...
										<?php 
											if (isset($_POST['submit'])){
												...
												$sql = "SELECT * FROM pump_tbl WHERE pump_name = '".$_name."' ";
												$result = mysqli_query($link,$sql);
												$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
												$count = mysqli_num_rows($result);
												
												if($count == 1) {
													echo "<script>alert ('Pump name already exists')</script>";
													
													
												}else{
													$sql = "INSERT INTO pump_tbl (pump_name,tank_name, product_name) VALUES ('".$_name."','".$_type."', '".$product."')";
			
													if (mysqli_query($link, $sql)) {
														echo "<script>alert ('Pump successfully added')</script>";
